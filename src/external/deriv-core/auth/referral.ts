 ...
 *
 * 4. *-tracking.deriv.com/click?a=AFFILIATE_ID (Scaleo)
 *    → static utm params extracted without redirect.
 */
export function parseReferralLink(referralLink: string): ReferralInfo | null {
  ...
  try {
    ...
    // Format 4: Scaleo click tracking link — *-tracking.deriv.com/click?a=AFFILIATE_ID
    // e.g. https://partner-sandbox-tracking.deriv.com/click?a=4346&o=1&c=4&link_id=1
    // utm_campaign=scaleo, utm_medium=affiliate, utm_source=affiliate_{a} are static.
    if (
      url.hostname.endsWith('-tracking.deriv.com') &&
      url.pathname === '/click'
    ) {
      ...
      if (affiliateId) {
        return {
          ...
          affiliateTokenParam: 't',
          utmCampaign: 'scaleo',
          utmSource: `affiliate_${affiliateId}`,
          utmMedium: 'affiliate',
        };
      }
    }
